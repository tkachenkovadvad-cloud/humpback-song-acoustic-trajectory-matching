"""
trajectory_similarity_matcher_v0_6.py
...
def _count_complete_signals(folder: Path) -> int:
    """Count frame exports that have usable event data in the same group folder."""
    frame_files = sorted(folder.glob("*_frames_shared_space.csv"))
    if not frame_files:
        return 0

    combined = folder / "all_signals_events_centroids_shared_space.csv"
    combined_names: set[str] = set()
    if combined.exists():
        try:
            df = pd.read_csv(combined, usecols=lambda c: c in {
                "source_file", "signal", "signal_name", "filename", "file"
            })
            name_col = next(
                (c for c in ("source_file", "signal", "signal_name", "filename", "file")
                 if c in df.columns),
    ...
    count = 0
    for frame_file in frame_files:
        stem = frame_file.name.removesuffix("_frames_shared_space.csv")
        event_file = folder / f"{stem}_events_centroids_shared_space.csv"
        if event_file.exists():
            count += 1
            continue
        try:
            frames = pd.read_csv(frame_file, nrows=1)
            original = _source_name_from_table(frames, stem)
        except Exception:
            original = stem
        if original in combined_names or stem in combined_names:
            count += 1
    return count
...
def _run_one_group(
    ...
    """
    Run the original single-folder matcher in a clean child process.
    --top is intentionally huge so every pair in the matching group receives
    the full permutation test.
    """
    cmd = [
        sys.executable,
        str(script_path),
        ...
        "--top", "1000000000",
        "--max-pair-minutes", str(max_pair_minutes),
        "--figures", figure_mode,
    ]
    if no_resume:
    ...
    env = os.environ.copy()
    env["TRAJECTORY_MATCHER_SINGLE_FOLDER"] = "1"
    completed = subprocess.run(cmd, env=env)
    return int(completed.returncode)


def _collect_record_results(
    ...
    for group in range(1, max_group + 1):
        folder = trajectory_root / str(group)

        final_file = folder / "trajectory_similarity_matches.csv"
        if final_file.exists():
            try:
                df = pd.read_csv(final_file)
                df.insert(0, "matching_group", group)
                df.insert(0, "record", record_id)
                result_parts.append(df)
            except Exception as exc:
                print(f"WARNING: could not collect {final_file}: {exc}", file=sys.stderr)

        screening_file = folder / "trajectory_similarity_screening_all_pairs.csv"
        if screening_file.exists():
            try:
                df = pd.read_csv(screening_file)
                df.insert(0, "matching_group", group)
                df.insert(0, "record", record_id)
                screening_parts.append(df)
            except Exception as exc:
                print(f"WARNING: could not collect {screening_file}: {exc}", file=sys.stderr)

    results = pd.concat(result_parts, ignore_index=True) if result_parts else pd.DataFrame()
    screening = pd.concat(screening_parts, ignore_index=True) if screening_parts else pd.DataFrame()

    results.to_csv(
        trajectory_root / "trajectory_similarity_matches_all_groups.csv",
        index=False,
    )
    screening.to_csv(
        trajectory_root / "trajectory_similarity_screening_all_groups.csv",
        index=False,
    )
    return results, screening


def batch_main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run the trajectory similarity matcher across matching-group folders."
        )
    )
    parser.add_argument(
        "--record1-root",
        type=Path,
        help="Trajectory root for record 1 (folders 1..N).",
    )
    parser.add_argument(
        "--record2-root",
        type=Path,
        help="Trajectory root for record 2 (folders 1..N).",
    )
    parser.add_argument(
        "--record",
        choices=["all", "1", "2"],
        default="all",
        help="Process both records or only one record.",
    )
    parser.add_argument(
        "--record1-groups",
        type=int,
        default=0,
        help="Highest matching-group folder number for record 1; 0 = auto-detect numeric folders.",
    )
    parser.add_argument(
        "--record2-groups",
        type=int,
        default=0,
        help="Highest matching-group folder number for record 2; 0 = auto-detect numeric folders.",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
        default=None,
        help="Folder for combined cross-record CSV outputs. Defaults to the common parent of record roots.",
    )
    parser.add_argument(
        "--permutations",
        type=int,
        default=N_PERMUTATIONS,
        help="Temporal-order null permutations for every pair.",
    )
    parser.add_argument(
        "--max-pair-minutes",
        type=float,
        default=30.0,
        help="Maximum time per pair; 0 disables the limit.",
    )
    parser.add_argument(
        "--figures",
        default="strong",
        choices=["none", "strong", "match", "all", "0", "1"],
        help="PNG output mode inside every matching-group folder.",
    )
    parser.add_argument(
        "--no-resume",
        action="store_true",
        help="Recalculate pairs even when prior partial/final results exist.",
    )
    args = parser.parse_args()

    figure_mode = {"0": "none", "1": "all"}.get(args.figures, args.figures)
    def detect_max_group(root: Path | None, explicit: int) -> int:
        if explicit > 0:
            return explicit
    ...
    jobs = []
    if args.record in {"all", "1"}:
        if args.record1_root is None:
            parser.error("--record1-root is required when processing record 1.")
        jobs.append((1, args.record1_root, detect_max_group(args.record1_root, args.record1_groups)))
    if args.record in {"all", "2"}:
        if args.record2_root is None:
            parser.error("--record2-root is required when processing record 2.")
        jobs.append((2, args.record2_root, detect_max_group(args.record2_root, args.record2_groups)))
    ...
    print("Trajectory similarity batch matcher")
    print("All pairs inside every matching group receive full permutation tests.")
    print(f"Permutations per pair: {max(99, args.permutations)}")
    print(f"PNG mode: {figure_mode}")

    for record_id, trajectory_root, max_group in jobs:
        if not trajectory_root.exists():
            print(f"ERROR: trajectory root does not exist: {trajectory_root}", file=sys.stderr)
            status_rows.append({
                "record": record_id,
                "matching_group": "",
                "folder": str(trajectory_root),
                "complete_signals": 0,
        ...
        print(f"\n=== RECORD {record_id}: {trajectory_root} ===")
        for group in range(1, max_group + 1):
            folder = trajectory_root / str(group)
            signal_count = _count_complete_signals(folder) if folder.exists() else 0

            if not folder.exists():
                status = "GROUP_FOLDER_NOT_FOUND"
                code = ""
                print(f"Record {record_id}, group {group:02d}: folder missing")
            elif signal_count < 2:
                status = "FEWER_THAN_TWO_COMPLETE_SIGNALS"
                code = ""
                print(
                    f"Record {record_id}, group {group:02d}: "
                    f"skipped ({signal_count} complete signal)"
                )
            else:
                print(
                    f"\nRecord {record_id}, group {group:02d}: "
                    f"{signal_count} signals"
                )
                code = _run_one_group(
                    script_path=script_path,
                    folder=folder,
                    permutations=max(99, args.permutations),
                    max_pair_minutes=args.max_pair_minutes,
                    figure_mode=figure_mode,
                    no_resume=args.no_resume,
                )
                status = "COMPLETE" if code == 0 else "MATCHER_ERROR"

            status_rows.append({
                "record": record_id,
                "matching_group": group,
                "folder": str(folder),
                "complete_signals": signal_count,
                "status": status,
                "return_code": code,
            })

        record_results, record_screening = _collect_record_results(
            trajectory_root, record_id, max_group
        )
        if not record_results.empty:
            overall_results.append(record_results)
        if not record_screening.empty:
            overall_screening.append(record_screening)

        pd.DataFrame(
            [r for r in status_rows if r["record"] == record_id]
        ).to_csv(
            trajectory_root / "trajectory_similarity_batch_status.csv",
            index=False,
        )

    if args.output_root is not None:
        common_root = args.output_root
    else:
        roots = [r.resolve() for _, r, _ in jobs]
        common_root = Path(__import__("os").path.commonpath([str(r.parent) for r in roots]))
    common_root.mkdir(parents=True, exist_ok=True)
    if overall_results:
        all_results = pd.concat(overall_results, ignore_index=True)
        all_results.to_csv(
            common_root / "trajectory_similarity_matches_both_records.csv",
            index=False,
        )
    if overall_screening:
        all_screen = pd.concat(overall_screening, ignore_index=True)
        all_screen.to_csv(
            common_root / "trajectory_similarity_screening_both_records.csv",
            index=False,
        )

    status = pd.DataFrame(status_rows)
    status_out = common_root / "trajectory_similarity_batch_status_both_records.csv"
    status.to_csv(status_out, index=False)

    errors = status["status"].isin(
        ["ROOT_NOT_FOUND", "GROUP_FOLDER_NOT_FOUND", "MATCHER_ERROR"]
    ).sum() if not status.empty else 1

    print("\nBATCH COMPLETE")
    print(f"Status: {status_out}")
    for record_id, trajectory_root, _ in jobs:
        print(
            f"Record {record_id} final combined CSV: "
            f"{trajectory_root / 'trajectory_similarity_matches_all_groups.csv'}"
        )
    print(
        "Both records final combined CSV: "
        f"{common_root / 'trajectory_similarity_matches_both_records.csv'}"
    )
    return 1 if errors else 0


def main() -> int:
    if os.environ.get("TRAJECTORY_MATCHER_SINGLE_FOLDER") == "1":
        return single_folder_main()
    return batch_main()
